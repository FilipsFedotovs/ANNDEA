...
while status<4:
      ...
      if status==3:
         print(bcolors.HEADER+"#############################################################################################"+bcolors.ENDC)
         print(UF.TimeStamp(),bcolors.BOLD+'Stage '+status+':'+bcolors.ENDC+' Taking the list of seeds previously generated by Stage 2, converting them into Emulsion Objects and doing more rigorous selection')
         JobSet=[]
         JobSets=Meta.JobSets
         for i in range(len(JobSets)):
             JobSet.append([])
              ...
              if (TotJobs)==len(bad_pop):
                 ...
                 if UserAnswer=='R':
                    ...
                    if AutoPilot(600,10,Patience,AFS_DIR,EOS_DIR,'/ANNADEA/Data/REC_SET/','RefinedSeeds','RUTr1b','.pkl',RecBatchID,JobSet,OptionHeader,OptionLine,'RUTr1b_RefineSeeds_Sub.py',['',''],False,False):
                        ...
                        print(UF.TimeStamp(),bcolors.FAIL+'Stage '+status+' is uncompleted...'+bcolors.ENDC)
                        status=8
                        break
