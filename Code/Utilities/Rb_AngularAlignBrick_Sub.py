...
def FitPlateFixedX(x):
    return UA.FitPlateAngle(Plate[i][0],x,0,train_data,'Rec_Seg_ID',False)
def FitPlateFixedY(x):
    return UA.FitPlateAngle(Plate[i][0],0,x,train_data,'Rec_Seg_ID',False)
def FitPlateValX(x):
    return UA.FitPlateAngle(Plate[i][0],x,0,validation_data,'Rec_Seg_ID',False)
def FitPlateValY(x):
    return UA.FitPlateAngle(Plate[i][0],0,x,validation_data,'Rec_Seg_ID',False)
if len(train_data)>0:
