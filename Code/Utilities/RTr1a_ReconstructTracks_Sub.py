...
CheckPointFile_Ini=EOS_DIR+p+'/Temp_'+pfx+'_'+RecBatchID+'_'+str(X_ID_n)+'_'+str(Y_ID_n)+'/'+pfx+'_'+RecBatchID+'_'+o+'_'+str(X_ID_n)+'_'+str(Y_ID_n)+'_'+str(Z_ID_n) +'_CP_Ini.pkl'
...
if os.path.isfile(CheckPointFile_Tracking_TH) and os.path.isfile(CheckPointFile_Tracking_RP):
        ...
        UI.Msg('location','Loading checkpoint file ',CheckPointFile_Ini)
...
if Status=='Initialisation': #Here we load the file from the scratch
    ...
    torch_import=True
    if CheckPoint:
             print(UI.TimeStamp(),'Saving checkpoint 1...')
