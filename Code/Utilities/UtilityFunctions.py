         ...
         if ModelMeta.ModelArchitecture=='TCN':
            ...
            class GMM(torch.nn.Module):
                def __init__(self):
                    ...
                    torch.manual_seed(12345)
                    if len(HiddenLayer)==3:
                        self.conv1 = GMMConv(4 , HiddenLayer[0][0],dim=HiddenLayer[0][1],kernel_size=HiddenLayer[0][2])
                        self.conv2 = GMMConv(HiddenLayer[0][0],HiddenLayer[1][0],dim=HiddenLayer[1][1],kernel_size=HiddenLayer[1][2])
                        self.conv3 = GMMConv(HiddenLayer[1][0],HiddenLayer[2][0],dim=HiddenLayer[2][1],kernel_size=HiddenLayer[2][2])
                        self.lin = Linear(HiddenLayer[2][0],OutputLayer[1])
                    self.softmax = Softmax(dim=-1)
