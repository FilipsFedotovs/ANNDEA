           ...
           if ModelTrainingSaturation(test_input):
              ...
              exit()
                 ...
                 if Model_Meta.ModelType=='CNN':
                    Job=UF.CreateCondorJobs(AFS_DIR,EOS_DIR,'/ANNADEA/Data/TRAIN_SET/','N/A','MUTr2','N/A',ModelName,1,OptionHeader,OptionLine,'MUTr2_TrainModel_Sub.py',False,"['','']", True, True)[0]
                 else:
                    Job=UF.CreateCondorJobs(AFS_DIR,EOS_DIR,'/ANNADEA/Data/TRAIN_SET/','N/A','MUTr2','N/A',ModelName,1,OptionHeader,OptionLine,'MUTr2_TrainModel_Sub.py',False,"['','']", True, False)[0]
                ...
                HTCondorTag="SoftUsed == \"ANNADEA-MUTr2-"+ModelName+"\""
                UF.TrainCleanUp(AFS_DIR, EOS_DIR, 'MUTr2_'+ModelName, ['N/A'], HTCondorTag)
                if Model_Meta.ModelType=='CNN':
                    Job=UF.CreateCondorJobs(AFS_DIR,EOS_DIR,'/ANNADEA/Data/TRAIN_SET/','N/A','MUTr2','N/A',ModelName,1,OptionHeader,OptionLine,'MUTr2_TrainModel_Sub.py',False,"['','']", True, True)[0]
                else:
                    Job=UF.CreateCondorJobs(AFS_DIR,EOS_DIR,'/ANNADEA/Data/TRAIN_SET/','N/A','MUTr2','N/A',ModelName,1,OptionHeader,OptionLine,'MUTr2_TrainModel_Sub.py',False,"['','']", True, False)[0]
 ...
 if UserAnswer=='E':
    ...
    AutoPilot(int(UserAnswer),Wait,Patience)
     ...
     if os.path.isfile(Model_Meta_Path):
       ...
       if len(Model_Meta.TrainSessionsDataID)==len(Model_Meta.TrainSessionsData):
           if len(Model_Meta.TrainSessionsData[-1])==2 and len(Model_Meta.TrainSessionsData)>2:
              print(Model_Meta.TrainSessionsData[-2][1])
              test_input=Model_Meta.TrainSessionsData[-3][1]+Model_Meta.TrainSessionsData[-2][1]+Model_Meta.TrainSessionsData[-1]
           else:
              test_input=Model_Meta.TrainSessionsData[completion]
           print(test_input)
           exit()
