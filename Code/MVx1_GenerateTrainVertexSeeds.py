...
#Vertexing Module of the ANNDEA package
    ...
    if Program[Status][:6]!='Custom' and (Program[Status] in ModelName)==False:
        ...
        UI.UpdateStatus(Status+1,Meta,TrainSampleOutputMeta)
            ...
            for i in range(0,len(JobSet)):
                 output_file_location=EOS_DIR+'/ANNDEA/Data/TRAIN_SET/'+TrainSampleID+'/MVx1c_'+TrainSampleID+'_CompressedSeeds_'+str(i)+'.pkl'
                 if os.path.isfile(output_file_location)==False:
                    if os.path.isfile(EOS_DIR+'/ANNDEA/Data/TRAIN_SET/'+TrainSampleID+'/MVx1c_'+TrainSampleID+'_Temp_Stats.csv')==False:
                       UI.LogOperations(EOS_DIR+'/ANNDEA/Data/TRAIN_SET/'+TrainSampleID+'/MVx1c_'+TrainSampleID+'_Temp_Stats.csv','w', [[0,0]])
                    ...
                    for j in range(JobSet[i]):
                                  required_output_file_location=EOS_DIR+'/ANNDEA/Data/TRAIN_SET/'+TrainSampleID+'/Temp_MVx1b_'+TrainSampleID+'_'+str(i)+'/MVx1b_'+TrainSampleID+'_'+'RefinedSeeds'+'_'+str(i)+'_'+str(j) + '.pkl'
                    ...
                    try:
                        ...
                        continue
                    del new_data
                    UI.LogOperations(EOS_DIR+'/ANNDEA/Data/TRAIN_SET/'+TrainSampleID+'/MVx1c_'+TrainSampleID+'_Temp_Stats.csv','w', [[TotalImages,TrueSeeds]])
...
if Status<20:
    ...
    print(UI.ManageFolders(AFS_DIR, EOS_DIR, TrainSampleID,'d',['MVx1a','MVx1b','MVx1c','MVx1d']))
    UI.Msg('success',"Vertexing training smaples have been generated")
else:
