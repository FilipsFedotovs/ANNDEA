...
# Purpose: To set up all working directories for the new user
...
print(
    bcolors.HEADER + "######################     Initialising ANNDEA Installation Module                 #####################" + bcolors.ENDC)
print(
    bcolors.HEADER + "#########################              Written by Filips Fedotovs              #########################" + bcolors.ENDC)
print(
...
def FolderCreate(DIR):
    try:
        ...
        print(bcolors.FAIL + 'Problem creating ' + bcolors.ENDC, bcolors.OKBLUE + DIR + bcolors.ENDC,
              bcolors.FAIL + 'folder, probably it already exists...' + bcolors.ENDC)
    ...
    for row in dir_reader:
        if row[0] == 'EOS_DIR':
            try:
                ...
                print(bcolors.FAIL + 'Problem removing ' + bcolors.ENDC, bcolors.OKBLUE + DEL_DIR + bcolors.ENDC,
                      bcolors.FAIL + 'folder, skipping the step...' + bcolors.ENDC)
    try:
        shutil.rmtree('HTCondor')
    except:
        print(bcolors.FAIL + 'Problem removing ' + bcolors.ENDC, bcolors.OKBLUE + 'HTCondor' + bcolors.ENDC,
              bcolors.FAIL + 'folder, skipping the step...' + bcolors.ENDC)
    try:
        shutil.rmtree('Code')
    except:
        print(bcolors.FAIL + 'Problem removing ' + bcolors.ENDC, bcolors.OKBLUE + 'Code' + bcolors.ENDC,
              bcolors.FAIL + 'folder, skipping the step...' + bcolors.ENDC)
    try:
        os.remove('config')
    except:
        print(bcolors.FAIL + 'Problem removing ' + bcolors.ENDC, bcolors.OKBLUE + 'Config' + bcolors.ENDC,
              bcolors.FAIL + 'file, skipping the step...' + bcolors.ENDC)

    print(
        bcolors.OKGREEN + 'Uninstallation complete, you can delete setup.py and its parent directory manually if you wish' + bcolors.ENDC)
    exit()
